...
cam_param_lst = torch.from_numpy(np.array([[[2.2901e+00, 2.2876e+00, 2.5083e-02, 2.8903e-02, -2.0710e-01,
                                             2.4778e-01, -3.0752e-03, -9.7570e-04, -1.4245e-03],
                                            [2.2994e+00, 2.2952e+00, 1.7697e-02, 1.6130e-02, -1.9421e-01,
                                             2.4041e-01, 6.8200e-03, -1.6190e-03, -2.7409e-03],
                                            [2.2983e+00, 2.2976e+00, 3.9632e-02, 2.8053e-03, -2.0834e-01,
                                             2.5549e-01, -2.4605e-03, 1.4844e-03, -7.6000e-04],
                                            [2.2910e+00, 2.2895e+00, 2.9936e-02, 1.7640e-03, -1.9838e-01,
                                             2.1832e-01, -8.9478e-03, -5.8721e-04, -1.8134e-03]]]))
...
def create_2d_data(data_path, dataset):
    keypoints = np.load(data_path, allow_pickle=True).tolist()
    # adjust the length of 2d data
    for subject in dataset.subjects():
        for action in dataset[subject].keys():
...
def fetch_kpts(subjects, dataset, keypoints, action_filter=None, parse_3d_poses=True):
    out_poses_3d = []
    out_poses_2d = []
    out_actions = []
    out_act_cls = []
    ...
    for subject in subjects:
        for action in keypoints[subject].keys():
            ...
            for i in range(len(poses_2d)):  # Iterate across cameras
                ...
                out_actions.append([action + '_' + subject + '_v' + str(i)] * poses_2d[i].shape[0])
                out_act_cls.append([action.split(' ')[0]] * poses_2d[i].shape[0])
    ...
    return out_poses_3d, out_poses_2d, out_actions, out_act_cls


############################
# Data loader
class PoseDataLoader(Dataset):
    def __init__(self, poses_3d, poses_2d, actions, act_cls, kpt_left, kpt_right, aug=False):
        assert poses_3d is not None

        self.aug = aug
        self.kpt_left = kpt_left
        self.kpt_right = kpt_right

        self._poses_3d = np.concatenate(poses_3d)
        self._poses_2d = np.concatenate(poses_2d)
        self._actions = reduce(lambda x, y: x + y, actions)
        self._act_cls = reduce(lambda x, y: x + y, act_cls)
    ...
    def __getitem__(self, index):
        ...
        out_action = self._act_cls[index]
        out_action_with_index = self._actions[index]

        # flipping augmentation
        if self.aug and np.random.uniform(0, 10) <= 5:
            out_pose_2d[:, 0] *= -1
            out_pose_2d[self.kpt_left + self.kpt_right] = out_pose_2d[self.kpt_right + self.kpt_left]
            out_pose_3d[:, 0] *= -1
            out_pose_3d[self.kpt_left + self.kpt_right] = out_pose_3d[self.kpt_right + self.kpt_left]

        # normalize
        out_pose_3d -= out_pose_3d[:1]
        out_pose_2d = torch.from_numpy(out_pose_2d).float()
        out_pose_3d = torch.from_numpy(out_pose_3d).float()

        return out_pose_3d, out_pose_2d, out_action, out_action_with_index
